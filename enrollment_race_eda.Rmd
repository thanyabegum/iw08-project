...
enr_1921 <- enr_1921 %>% mutate(
    "Total Enrollment Change" = percent_change(`Total Enrollment.x`, `Total Enrollment.y`),
    "White Change" = percent_change(White.x, White.y),
    "Black Change" = percent_change(Black.x, Black.y),
    "Hispanic Change" = percent_change(Hispanic.x, Hispanic.y),
    "Asian Change" = percent_change(Asian.x, Asian.y),
    "Native American Change" = percent_change(`Native American.x`, `Native American.y`),
    "Hawaiian Native Change" = percent_change(`Hawaiian Native.x`, `Hawaiian Native.y`),
    "Two Or More Races Change" = percent_change(`Two or More Races.x`, `Two or More Races.y`),
    "%Free Lunch Change" = `%Free Lunch.y` - `%Free Lunch.x`,
    "%Reduced Lunch Change" = `%Reduced Lunch.y` - `%Reduced Lunch.x`
)

# Select only the columns that matter
enr_1921 <- enr_1921 %>% select(
    `County Code.x`, `County Name.x`, `District Code`, `District Name.x`,
    `Total Enrollment Change`, `White Change`, `Black Change`,
    `Hispanic Change`, `Asian Change`, `Native American Change`,
    `Hawaiian Native Change`, `Two Or More Races Change`, `%Free Lunch Change`,
    `%Reduced Lunch Change`
)

# Rename those columns
names(enr_1921)[1:4] <- c("County Code", "County Name", "District Code", "District Name")
...
# Export as csv
write.csv(enr_1921, file="output/enrollment_1921_change.csv", row.names=FALSE)
